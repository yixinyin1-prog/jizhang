...
const Parser = (() => {
  ...
  // 收入特征（覆盖常见说法：薪资/发薪/提成/佣金/退税/中奖/卖出 等都算）
  const INCOME_RE = /(红包入|红包\s*\+|红包收|收入|入账|进账|工资|薪资|薪水|发工资|发薪|月薪|提成|佣金|加班费|津贴|补贴|公积金|退税|中奖|稿费|外快|兼职|卖了|卖出|转进|给我|收到|收款|退款|返现|退回|报销到账|到账|好评\s*\+|收益|分红|利息|股息|奖金|年终奖)/;
  // 明确支出特征（覆盖收入判断，例如 "给小倩报销 80" 是支出）。
  // 注意排除「给我 / 还我」——那是别人给钱、还钱，属于收入方向。
  const EXPENSE_OVERRIDE_RE = /^(给(?!我)|发红包|还(?!我)|借给|买)/;

  /* 计算 "8.31+7.84" / "755+90=845" / "20-6.42" 这类金额表达式 */
  function evalAmount(seg) {
    // 「1.2万 / 1万2」这类万位写法先展开
    const wan = seg.match(/(\d+(?:\.\d+)?)\s*万(?:\s*(\d))?/);
    if (wan) {
      const v = parseFloat(wan[1]) * 10000 + (wan[2] ? parseFloat(wan[2]) * 1000 : 0);
